...
void lv_demo_widgets(void)
{
    ...
    lv_style_init(&style_icon);//不同类型配置不同的结构体
    lv_style_set_text_color(&style_icon, lv_theme_get_color_primary(NULL));
    lv_style_set_text_font(&style_icon, font_large);

    lv_style_init(&style_bullet);
    lv_style_set_border_width(&style_bullet, 0);//bullet 子弹
    ...
    if(disp_size == DISP_LARGE) {
        lv_obj_t * tab_btns = lv_tabview_get_tab_btns(tv);//返回一个按钮矩阵  列表
        ...
        lv_label_set_text(label, "Widgets demo");//小字
    ...
    lv_obj_t* t4 = lv_tabview_add_tab(tv, "some");
    profile_create(t1);//实现主体绑定
    analytics_create(t2);
    shop_create(t3);
    shop_create(t4);
...
static void profile_create(lv_obj_t * parent)
{
    lv_obj_t * panel1 = lv_obj_create(parent); //如何定位
    ...
    lv_label_set_long_mode(dsc, LV_LABEL_LONG_WRAP);//文字排序模式
